...
func (p *XSSPlugin) Scan(ctx context.Context, pURL models.ParameterizedURL, aiPayloads []string) ([]Vulnerability, error) {
	...
	for _, payload := range allPayloads {
		...
		for _, vp := range vulnerableParams {
			...
			log.Info().Str("type", "XSS").Str("url", pURL.URL).Str("param", vp).Msg("Vulnerability Found!")
		}
	}
...
func (p *XSSPlugin) checkDOMContext(ctx context.Context, req *models.Request, paramToRandomStr map[string]string) []string {
