...
// 它封装了Go标准库的http.Client，并提供了额外的便利功能，
// 如默认头部、代理支持和用于常见请求类型（GET, POST, POST Form, POST JSON）的辅助方法。
package requester
...
// 它被设计为可重用的，并可以在多个goroutine之间安全地共享。
...
// Post 是一个通用的辅助方法，用于发送POST请求，支持任意的Content-Type和body。
...
// PostForm 是一个专门用于发送 "application/x-www-form-urlencoded" 类型POST请求的辅助方法。
...
// PostJSON 是一个专门用于发送 "application/json" 类型POST请求的辅助方法。
